...
@Suite struct `ASCII Code Array Tests` {
    @Suite struct Unit {
        ...
        @Test func `init ascii string fails for non-ASCII input`() {
            #expect([ASCII_Primitives.ASCII.Code](ascii: "café") == nil)
        }

        @Test func `init ascii lineEnding produces the matching code sequence`() {
            #expect([ASCII_Primitives.ASCII.Code](ascii: .lf) == [.lf])
            #expect([ASCII_Primitives.ASCII.Code](ascii: .cr) == [.cr])
            #expect([ASCII_Primitives.ASCII.Code](ascii: .crlf) == [.cr, .lf])
        }
        ...
        @Test func `static ascii whitespaces contains the four INCITS 4-1986 whitespace codes`() {
            let whitespaces = [ASCII_Primitives.ASCII.Code].ascii.whitespaces
    ...
    @Suite struct `Edge Case` {
        @Test func `init ascii string succeeds for an empty string`() {
            #expect([ASCII_Primitives.ASCII.Code](ascii: "") == [])
        }

        @Test func `static ascii unchecked on an empty string is empty`() {
            #expect([ASCII_Primitives.ASCII.Code].ascii.unchecked("").isEmpty)
        }

        @Test func `static ascii whitespaces does not contain a non-whitespace code`() {
            #expect(![ASCII_Primitives.ASCII.Code].ascii.whitespaces.contains(.A))
        }
    }
